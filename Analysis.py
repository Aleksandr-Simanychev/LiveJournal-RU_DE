...
for post in data_df['Post']:
    if data_df['Date'][post_index] == 2014:
        try:
            ...
            print('тут был трабл')
        try:
            post = re.sub(r'ru_de[a-zA-Z]*[а-яА-ЯёЁ]*[ ]', '', post)
        except:
            print('тут был трабл')
        try:
            post = re.sub(r'edited at -- :', '', post)
        except:
            print('тут был трабл')
        try:
        ...
        Intersentence = re.findall(r'[а-яА-ЯёЁ]+[.?!:;][ ]?[a-zA-Z]+[ ][a-zA-Z]+]', str(Sentences_of_post))
        Intersentence_1 = re.findall(r'[a-zA-Z]+[ ][a-zA-Z]+[.?!:;][ ]?[а-яА-ЯёЁ]+', str(Sentences_of_post))
